...
function renderShopping(){
  ...
  const rowHtml=x=>{
    const manual=isManualShoppingItem(x),sources=sourceCount(x);
    const meta=manual?'Manuell hinzugefügt':`aus ${sources} Rezept${sources===1?'':'en'}${x.note?` · ${esc(x.note)}`:''}`;
    return `<div class="shopping-row ${x.done?'is-done':''} ${manual?'is-manual':''}" data-shopping-key="${esc(x.key)}"><input class="shopping-check" type="checkbox" ${x.done?'checked':''} data-shop-done="${esc(x.key)}" aria-label="Erledigt"><div class="shopping-name"><strong>${esc(x.article)}</strong><small>${meta}</small></div><div class="shopping-amount">${esc(displayAmount(x))}</div><details class="shopping-row-menu"><summary aria-label="Weitere Aktionen für ${esc(x.article)}" title="Weitere Aktionen">•••</summary><div class="shopping-row-menu-panel" data-shop-row-menu-panel></div></details>${manual?`<button class="shopping-action-proxy" type="button" data-shop-edit="${esc(x.key)}" hidden>Bearbeiten</button>`:''}<button class="shopping-action-proxy" type="button" data-shop-remove="${esc(x.key)}" hidden>Entfernen</button></div>`;
  };
  app.innerHTML=`<div class="list-page"><div class="shell"><div class="list-head"><div><span class="category">Gespeichert auf diesem Gerät</span><h1>Einkaufsliste</h1><p>${rows.length?`${rows.length} Einkaufsartikel`:'Noch keine Artikel hinzugefügt.'}</p></div>${rows.length?'<div class="list-actions"><button type="button" id="clearShopping" class="shopping-action-proxy" hidden>Liste leeren</button></div>':''}</div>${manualForm}${rows.length?`<div class="shopping-list">${rows.map(rowHtml).join('')}</div>`:'<div class="shopping-empty">Zutaten lassen sich direkt aus einem Rezept übernehmen. Andere Dinge kannst du oben manuell hinzufügen.</div>'}</div></div>`;
