...
async function snapshot(){
  ...
  return {format:FORMAT,version:BACKUP_VERSION,exportedAt:new Date().toISOString(),app:'Gerds Rezepte',stores:Object.fromEntries(entries),localStorage:local};
}
function counts(data){
  const stores=data?.stores||{};
  return {favorites:Array.isArray(stores.favorites)?stores.favorites.length:0,shopping:Array.isArray(stores.shopping)?stores.shopping.length:0,notes:Array.isArray(stores.recipeNotes)?stores.recipeNotes.length:0,plans:Array.isArray(stores.recipePlans)?stores.recipePlans.length:0,timers:Array.isArray(stores.timers)?stores.timers.length:0};
}
...
async function restore(data){
  const db=await openDb(),available=STORES.filter(name=>db.objectStoreNames.contains(name));
  ...
  await new Promise((resolve,reject)=>{
    ...
    for(const name of available){const os=tx.objectStore(name);os.clear();const rows=Array.isArray(data.stores[name])?data.stores[name]:[];rows.forEach(row=>os.put(row))}
    tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error||new Error('Import fehlgeschlagen.'));tx.onabort=()=>reject(tx.error||new Error('Import wurde abgebrochen.'));
  });
  for(const key of STORAGE_KEYS){
    try{const value=data.localStorage?.[key];if(value===undefined||value===null)localStorage.removeItem(key);else localStorage.setItem(key,typeof value==='string'?value:JSON.stringify(value))}
...
function download(name,text){
  ...
  link.href=url;link.download=name;link.hidden=true;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
}
function dateLabel(value){const date=new Date(value);return Number.isNaN(date.getTime())?'unbekannt':new Intl.DateTimeFormat('de-DE',{dateStyle:'medium',timeStyle:'short'}).format(date)}
function ensureUi(){
...
function bind(){
  ...
  if(trigger&&!trigger.dataset.bound){trigger.dataset.bound='1';trigger.addEventListener('click',openDialog)}
